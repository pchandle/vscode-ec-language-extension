...
Each contract result has **Open specification** and an expression action. **Open expression** opens the matching `job`; if more than one expression contains that contract job, choose the required job from a file-and-line picker. When no matching job is indexed, the action becomes **New expression** and creates a starter job. Source actions reuse the active editor group by default; Ctrl-click on Windows/Linux or Cmd-click on macOS opens beside instead, including for a newly created expression. Its tooltip shows the resolved target. The target directory is built from `specification.defaultContractExpressionPath` beneath the configured Component Manager directory containing the contract, while its filename and extension use `specification.contractFilenameFormat` and `emergent.autopilotExtension`.
