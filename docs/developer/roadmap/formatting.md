...
### Last Completed Step
- Block 3 slice: attached standalone `//` comment lines to parsed multiline `if` headers and trailing `end -> ...` target continuations. Parsed multiline `if` continuations now indent directly adjacent standalone comment groups to the same continuation depth as the owned header and trailing target lines.
- Scope decision: this slice is limited to standalone `//` comment groups directly adjacent to parser-owned multiline `if` header lines and parsed trailing `end -> ...` target continuation lines. It does not yet expand comment attachment to parsed multiline `defaults`, `job` / `def` headers or bodies, inline comments, malformed recovery ownership, broader blank-line policy, or the still-unowned gap where a standalone comment appears between a newline-after-`->` line and the first braced obligation opening.
...
- Lesson captured for future PRs: parser ownership boundaries differ across multiline statement families, so each comment-attachment slice should be validated against real parse-success inputs before extending the same policy to another owner family.
