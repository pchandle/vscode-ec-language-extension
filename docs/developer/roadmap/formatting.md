...
### Next Recommended PR
- Block 3: extend the same parse-success-only comment-attachment model to another explicit multiline ownership slice that still lacks standalone comment propagation, most likely a remaining parser-backed declaration/header-or-body comment ownership gap, while continuing to keep malformed recovery formatting conservative.
