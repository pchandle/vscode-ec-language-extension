...
### Last Completed Step
- Block 3 slice: attached standalone `//` comment lines to the parser-backed subset of parsed multiline `job` / `def` headers. Parsed declaration headers now indent directly adjacent standalone comment groups to the same continuation depth where the parser still keeps those comment-bearing header lines inside the declaration header ownership boundary.
- Scope decision: this slice is limited to standalone `//` comment groups directly adjacent to parser-owned multiline `def` header continuation lines and the parameter-list side of multiline `job` headers. It does not yet expand comment attachment to parsed multiline `defaults`, `job` target-list header continuations, declaration bodies, inline comments, malformed recovery ownership, broader blank-line policy, or the still-unowned gap where a standalone comment appears between a newline-after-`->` line and the first braced obligation opening.
...
### Next Recommended PR
- Block 3: extend the same parse-success-only comment-attachment model to another explicit multiline ownership slice, most likely declaration-body attachment for parsed `job` / `def` regions or parser-backed support for the still-unowned `job` target-list header continuation gap, while continuing to keep malformed recovery formatting conservative.
