...
### Last Completed Step
- Block 3 slice: added parser-backed support for the multiline `job` target-list header continuation gap caused by standalone comment lines, and extended the existing parsed declaration-header comment attachment coverage across that newly owned region. Parsed multiline `job` headers now keep comment-separated target continuations inside the header ownership boundary, so directly adjacent standalone comment groups indent to the same continuation depth as the owned header lines.
- Scope decision: this slice is limited to the parser-backed `job` target-list header continuation gap plus formatter coverage for standalone `//` comment groups directly adjacent to parser-owned multiline declaration-header continuation lines. It does not yet expand comment attachment to parsed multiline `defaults`, declaration bodies, inline comments, malformed recovery ownership, broader blank-line policy, or the still-unowned gap where a standalone comment appears between a newline-after-`->` line and the first braced obligation opening.
...
### Next Recommended PR
- Block 3: extend the same parse-success-only comment-attachment model to another explicit multiline ownership slice, most likely declaration-body attachment for parsed `job` / `def` regions or another parser-backed declaration/comment ownership gap, while continuing to keep malformed recovery formatting conservative.
