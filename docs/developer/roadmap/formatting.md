...
### Next Recommended PR
- Block 3: extend the same parse-success-only comment-attachment model to another explicit multiline ownership slice, most likely another parser-backed declaration/comment ownership gap or the still-unowned newline-after-`->` first-obligation comment gap, while continuing to keep malformed recovery formatting conservative.
